...
type NavIconType = 'dashboard' | 'marketplace' | 'volunteers' | 'finance' | 'nft' | 'training' | 'community' | 'photostudio';
...
export const NavIcon: React.FC<IconProps> = ({ type, className = "w-6 h-6" }) => {
  // FIX: Replaced JSX.Element with React.ReactElement to resolve "Cannot find namespace 'JSX'" error.
  const icons: Record<NavIconType, React.ReactElement> = {
    dashboard: (
    ...
    photostudio: (
      <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19.5 8.5l-1 1M4.5 8.5l1 1M12 18.5l.5.5M12 18.5l-.5.5M12 18.5l.5-.5M12 18.5l-.5-.5" />
      </svg>
    ),
    volunteers: (
      <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return <span className="flex items-center justify-center w-6">{icons[type]}</span>;
};

interface LogoIconProps {
  size?: 'normal' | 'large';
}
export const LogoIcon: React.FC<LogoIconProps> = ({ size = 'normal' }) => {
    const sizeClasses = {
    ...
    return (
        <div className={`bg-gradient-to-br from-teal-500 to-teal-600 ${sizeClasses[size]}`}>
            <svg className={`${svgSizeClasses[size]} text-white`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
